...
https://raw.githubusercontent.com/apache/incubator-airflow/v1-9-stable/airflow/operators/docker_operator.py
with slight modifications to

* use CustomDockerHook
"""
...
import airflow.hooks as hooks
import ast


class CustomDockerOperator(BaseOperator):
    """
    ...
    def __init__(
        ...
        super(CustomDockerOperator, self).__init__(*args, **kwargs)
    ...
    def execute(self, context):
        ...
        if self.docker_conn_id:
            self.log.info('Creating docker client from connection %s', self.docker_conn_id)
            self.cli = self.get_hook().get_conn()
        else:
            self.log.info('Creating docker client')
            self.cli = Client(
                base_url=self.docker_url,
                version=self.api_version,
                tls=tls_config
        ...
        with TemporaryDirectory(prefix='airflowtmp') as host_tmp_dir:
            ...
            self.log.info('Running container with %s', self.get_command())
            self.log.info('%s', self.get_command())
            self.log.info('%s', self.cli)
