...
if [ "${ENVIRONMENT_NAME}" = "prod" ]; then
  source ${PATH_TO_CONFIG_VAR_DIR}/_prod.sh
elif [ "${ENVIRONMENT_NAME}" = "stg" ]; then
  source ${PATH_TO_CONFIG_VAR_DIR}/_stg.sh
elif [ "${ENVIRONMENT_NAME}" = "dev" ]; then
  source ${PATH_TO_CONFIG_VAR_DIR}/_dev.sh
else
  echo "Error!"
...
source ${PATH_TO_CONFIG_VAR_DIR}/_common.sh
